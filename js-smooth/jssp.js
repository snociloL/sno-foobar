				...
				for (var i = g_start_; i <= g_end_; i++) {
					...
					case ghrh: // last group header row
						if (ay > 0 - (ghrh * ah) && ay < this.h + (ghrh * ah)) {
							...
							// =====
							// right area
							cColumns.dateWidth = gr.CalcTextWidth((arr_e[3] || "").substring(0, 4), g_font) + 10;
							gr.GdiDrawText((arr_e[3] || "").substring(0, 4), g_font, group_color_txt_normal, ax + aw - cColumns.dateWidth - 5, ay - ((ghrh - 1) * ah) + Math.round(ah * 1 / 3) - 2, cColumns.dateWidth, Math.round(ah * 2 / 3), DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
							cColumns.genreWidth = Math.min(gr.CalcTextWidth(arr_e[2], g_font_group2) + 10, Math.round((aw - text_left_margin - 30) * 0.55));
